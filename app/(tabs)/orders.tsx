...
// Интерфейс для задачи (для TypeScript)
interface Order {
  id: number;
  task_name: string;
  driver_id: string;
...
export default function OrdersScreen() {
  ...
  const filterOrders = () => {
    if (!searchQuery.trim()) {
      ...
      const filtered = orders.filter(
        (o) =>
          (o.task_name && o.task_name.toLowerCase().includes(query)) ||
  ...
  const handleEditOrder = async () => {
    ...
    try {
      const updated = await updateOrder(selectedOrder.id, editFormData);
  ...
  const handleDeactivateOrder = async (order: Order) => {
    Alert.alert(
      'Снять задачу',
      `Вы уверены, что хотите снять задачу "${order.task_name || order.customer}"?`,
      [
  ...
  const renderOrder = ({ item }: { item: Order }) => (
    <TouchableOpacity
      ...
      onLongPress={() => handleDeactivateOrder(item)}
    >
        ...
        <View style={styles.modalOverlay}>
          <ScrollView style={styles.modalContent} contentContainerStyle={styles.modalContentContainer}>
            ...
            <View style={styles.pickerContainer}>
              {drivers.map((d) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.pickerOption,
                    formData.driver_id === d.id && styles.pickerOptionSelected,
                  ]}
              ...
              <View style={styles.pickerContainer}>
                {customers.map((c) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.pickerOption,
                      formData.customer === c && styles.pickerOptionSelected,
                    ]}
            ...
            <View style={styles.row}>
              <View style={styles.halfInput}>
                ...
                <View style={styles.unitButtons}>
                  <TouchableOpacity
                    style={[styles.unitButton, formData.unit === 'м3' && styles.unitButtonActive]}
                    onPress={() => setFormData({ ...formData, unit: 'м3' })}
                  >
                    <Text>м3</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.unitButton, formData.unit === 'т' && styles.unitButtonActive]}
            ...
            <TextInput
              ...
              onChangeText={(text) => setFormData({ ...formData, company_rate: text })}
            />
            ...
            <TextInput
              ...
              onChangeText={(text) => setFormData({ ...formData, driver_rate: text })}
            />

            <TextInput
              style={styles.input}
              placeholder="Отправитель (опционально)"
            ...
            <TextInput
              style={styles.input}
              placeholder="Получатель (опционально)"
        ...
        <View style={styles.modalOverlay}>
          <ScrollView style={styles.modalContent} contentContainerStyle={styles.modalContentContainer}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Адрес погрузки"
              value={editFormData.load_address}
            ...
            <TextInput
              style={styles.input}
              placeholder="Адрес выгрузки"
              value={editFormData.unload_address}
            ...
            <View style={styles.row}>
              <View style={styles.halfInput}>
                ...
                <View style={styles.unitButtons}>
                  <TouchableOpacity
                    style={[styles.unitButton, editFormData.unit === 'м3' && styles.unitButtonActive]}
                    onPress={() => setEditFormData({ ...editFormData, unit: 'м3' })}
                  >
                    <Text>м3</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.unitButton, editFormData.unit === 'т' && styles.unitButtonActive]}
